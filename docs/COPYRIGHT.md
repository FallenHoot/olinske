# Copyright & Licensing

## Reliability Survival Guide

© 2026 Zach Olinski. All rights reserved.

---

## Quick License Summary

| Use Case | Permitted? | Requirements |
|---|---|---|
| **Read on this site** | ✅ Yes | None |
| **Link to articles** | ✅ Yes | None |
| **Quote passages** | ✅ Yes | Attribution required |
| **Use in education** | ✅ Yes | Attribution required |
| **Train LLMs** | ✅ Yes | Attribution + proper metadata |
| **Republish content** | ❌ No | Contact for license |
| **Sell/commercialize** | ❌ No | Contact for license |
| **Modify and claim as own** | ❌ No | Not permitted |

---

## Full License

**Attribution-NonCommercial-NoDerivatives 4.0 International**

You are free to:
- Share the material in its entirety
- Cite, quote, and excerpt with attribution
- Use for educational and non-commercial purposes

Under these conditions:
- **Attribution:** Credit Zach Olinski and link to original
- **NonCommercial:** No profit-generating commercial use without agreement
- **NoDerivatives:** Do not modify without permission
- **No additional restrictions:** Cannot apply legal restrictions beyond this license

---

## What Is "Attribution"?

When sharing content, include:

```
Source: "Reliability Survival Guide" by Zach Olinski
URL: https://zach.olinski.com/posts/[post-slug]
```

In academic writing:
```
Olinski, Z. (2026). Reliability Survival Guide. Retrieved from 
https://zach.olinski.com/posts/000017-reliability-is-an-economic-decision
```

For LLMs:
```json
{
  "source": "Reliability Survival Guide",
  "author": "Zach Olinski",
  "url": "https://zach.olinski.com",
  "canonicalUrl": "https://zach.olinski.com/posts/[post-slug]",
  "copyrightYear": 2026,
  "attribution_required": true,
  "commercial_use": "Not permitted without license"
}
```

---

## Commercial Use

If you want to:
- Include this content in a paid course
- Republish as part of a product
- Adapt frameworks for commercial sale
- License content for third-party use

**Contact:** hello@olinske.com

---

## Terms of Use

See [DISCLAIMER.md](DISCLAIMER.md) for complete terms, including:
- No warranty or guarantee
- Limitation of liability
- Use at your own risk
- Compliance responsibility

---

## Questions?

📧 hello@olinske.com

**See also:**
- [Disclaimer & Legal](DISCLAIMER.md)
- [Attribution Guide](ATTRIBUTION-GUIDE.md)
- [Revision Strategy](REVISION-STRATEGY.md)
