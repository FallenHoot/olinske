...
export const calculateCompositeAvailabilityPct = (catalog, selections) => {
...
export const downtimeBudgetFromPct = (availabilityPct) => {
...
export const evaluateUpgradeImpacts = (catalog, selections) => {
  const selectionMap = new Map(selections.map((entry) => [entry.serviceId, entry]));
...
export const computeSloPlan = (compositeSlaPct, companySlaPct, internalSloBufferBps) => {
...
export const formatPct = (value) => `${value.toFixed(6)}%`;
